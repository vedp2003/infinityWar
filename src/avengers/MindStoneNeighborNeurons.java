...
public class MindStoneNeighborNeurons {
    ...
    public static void main (String [] args) {
        ...
        String mindInputFile = args[0];
        StdIn.setFile(mindInputFile);
        String mindOutputFile = args[1];
        StdOut.setFile(mindOutputFile);

        int neurons = StdIn.readInt(); 

        String[] neuronNames = new String[neurons]; 
        for(int i = 0; i < neurons; i++){
            neuronNames[i] = StdIn.readString();
        }

        int synapses = StdIn.readInt(); 
        String[] dendrites = new String[synapses];
        String[] axons = new String[synapses];

        int val1 = 0;
        int val2 = 0; 

        for(int i = 1; i <= dendrites.length*2; i++){
            
            if(i%2 == 1){
                dendrites[val1++] = StdIn.readString();
            }
            else{
                axons[val2++] = StdIn.readString(); 
        ...
        for(String str: neuronNames){
            find = false;
            for(String str1: dendrites){
                if(str.equals(str1)){
                    find = true;
                    break;
                }
        ...
        for(int i = 0; i < axons.length; i++){
            if(axons[i].equals(mindStone)){
                StdOut.println(dendrites[i]); 
            }
        }
